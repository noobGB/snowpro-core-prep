...
|---|---|---|
| **Identity / login / password / multi-user (LAN) / forgot-password (email) / admin roles** | `pipeline/src/db.ts`, `pipeline/src/passwords.ts`, `pipeline/src/mailer.ts` (issue #59 — SMTP via `SNOWPRO_SMTP_*`; issue #62 adds `sendAdminCreatedAccountEmail`), `pipeline/src/server.ts` (`/api/session`, `/api/me`, `/api/logout`, `/api/account/password[-setup]`, `/api/password-reset/[request\|confirm]`, `/api/admin/*` routes, `requireAdmin`), `app/src/lib/session.ts`, `app/src/lib/admin.ts` (issue #62), `app/src/components/LoginGate.tsx`, `app/src/components/ResetPasswordPage.tsx`, `app/src/components/PasswordInput.tsx`, `app/src/components/SettingsPanel.tsx` (Profile section), `app/src/pages/Admin.tsx` (issue #62), `app/src/components/Sidebar.tsx` (Admin nav link, desktop), `app/src/components/MobileMoreSheet.tsx` + `app/src/components/MobileBottomNav.tsx` (Admin nav link, mobile — separate component tree from Sidebar.tsx, doesn't inherit its changes for free), `pipeline/scripts/admin-users.mjs` (operator CLI — list/remove/promote/demote/reset-all against the same `users`/`sessions`/`progress` tables), `.env.example` | README.md's `## Multi-user (LAN)` (including its admin-CLI, forgot-password, and admin bullets), `## Features` (Profile/Settings bullets), intro paragraph, `## Architecture`, `.github/screenshot-login.png`; TECH_STACK.md `## 5. Backend`; CLAUDE.md's Identity + Password login + Self-service password reset + Admin roles/page + Adding a user as admin + Admin CLI paragraphs under `## Frontend app (app/)`, and its `## Running it` `.env.example` + `SNOWPRO_HOST_NAME`/`publicOrigin()` mentions |
