...
|---|---|---|
| **Identity / login / multi-user (LAN)** | `pipeline/src/db.ts`, `pipeline/src/server.ts` (`/api/session`, `/api/me`, `/api/logout` routes), `app/src/lib/session.ts`, `app/src/components/LoginGate.tsx`, `app/src/components/SettingsPanel.tsx` (Profile section) | README.md's `## Multi-user (LAN)`, `## Features` (Profile/Settings bullets), intro paragraph, `## Architecture`; TECH_STACK.md `## 5. Backend`; CLAUDE.md's Identity paragraph under `## Frontend app (app/)` |
