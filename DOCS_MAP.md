...
| **Docker / Compose / deployment shape / HTTPS** | `Dockerfile`, `docker-compose.yml`, `Caddyfile` (issue #64), `pipeline/src/server.ts` (`trust proxy`, `issueSessionCookie()`'s `Secure` flag, `publicOrigin()`), `Launch-SnowPro.ps1`, `.github/workflows/ci.yml`'s `docker-smoke` job | CLAUDE.md `## Running it` (including its HTTPS via Caddy + Windows launcher paragraphs); TECH_STACK.md `## 7. Containerization` + `## 9. CI/CD`'s Docker smoke test paragraph; README.md's install instructions (`### Option A`) and Multi-user (LAN) section; `docs/architecture.drawio` (+ rendered image/GIF) — a new container/process in the request path |
