...
|---|---|---|
| **Identity / login / password / multi-user (LAN) / forgot-password (email)** | `pipeline/src/db.ts`, `pipeline/src/passwords.ts`, `pipeline/src/mailer.ts` (issue #59 — SMTP via `SNOWPRO_SMTP_*`), `pipeline/src/server.ts` (`/api/session`, `/api/me`, `/api/logout`, `/api/account/password[-setup]`, `/api/password-reset/[request\|confirm]` routes), `app/src/lib/session.ts`, `app/src/components/LoginGate.tsx`, `app/src/components/ResetPasswordPage.tsx`, `app/src/components/PasswordInput.tsx`, `app/src/components/SettingsPanel.tsx` (Profile section), `pipeline/scripts/admin-users.mjs` (operator CLI — list/remove/reset-all against the same `users`/`sessions`/`progress` tables), `.env.example` | README.md's `## Multi-user (LAN)` (including its admin-CLI and forgot-password bullets), `## Features` (Profile/Settings bullets), intro paragraph, `## Architecture`, `.github/screenshot-login.png`; TECH_STACK.md `## 5. Backend`; CLAUDE.md's Identity + Password login + Self-service password reset + Admin CLI paragraphs under `## Frontend app (app/)`, and its `## Running it` `.env.example` mention |
